...
class Menu extends Widget
{
     ...
     */
    public $linkNextTemplate = '<span class="fa arrow"></span>';

    public $linkLabel = '<span class="nav-label">{label}</span>';

    public $linkTemplate = '<a {class} href="{url}" title="{alt}">
                <i class="fa {ico}"></i>
                {label}
                {arrow}
                </a>';
     ...
     */
    protected function renderItems($items, $iNum = 1)
    {
        $iNum ++;
        $n = count($items);
        $iNumArray = [];
        ...
        foreach ($items as $i => $item) {
            ...
            $menu = $this->renderItem($item, $iNum);
            if (!empty($item['items'])) {
                $submenuTemplate = ArrayHelper::getValue($item, 'submenuTemplate', $this->submenuTemplate);

                $menu .= strtr($submenuTemplate, [
                    '{id}' => ($iNum <5 ? $this->templateCss[$iNum] : ''),
                    '{style}' => ($iNum <5 ? '' : 'style="padding:10px"'),
                    '{items}' => $this->renderItems($item['items'], $iNum),
                ]);
            }
     ...
     * @return string the rendering result
     */
    protected function renderItem($item, $iNum = 2)
    {
        $arrow = false;
        ...
        return strtr($template, [
            '{class}' => $arrow ? '' : 'class="J_menuItem"',
            '{ico}' => $arrow ? 'fa-desktop' : 'fa-table',
            '{url}' => $url,
            '{alt}' => $item['label'],
            '{label}' => $this->linkLabel($item['label'], $iNum),
            '{arrow}' => $arrow ? $this->linkNextTemplate : '',
        ]);
    }

    protected function linkLabel($label, $iNum)
    {
        if ($iNum == 2) {
            return strtr($this->linkLabel, [
                '{label}' => $label,
            ]);
        }
        else {
